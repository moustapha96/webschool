...
@section('data')
    <div class="card-block card-dashboard">
        <form class="form-group" action="{{ route('admin.semester.update', $semester) }}" method="POST">
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                    ...
                    <input type="text" name="code" value="{{ $semester->code }}"
                        class="form-control  @error('code') is-invalid @enderror" id="code" placeholder="Code semestre"
                        required>
                    @error('code')
                ...
                <div class="form-group col-md-12">
                    ...
                    <input type="text" name="name" value="{{ $semester->name }}"
                        class="form-control  @error('name') is-invalid @enderror" id="name" placeholder="nom du semestre"
                        required>
                    @error('name')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                ...
                <div class="form-group col-md-12">
                    ...
                    <select id="inputState" name="class_id" class="form-control">
                        ...
                        @foreach ($classes as $x)
                            <option value="{{ $x->id }}" {{ $x->id == $semester->class_id ? 'selected' : '' }}>
                                {{ $x->niveau->code }}
