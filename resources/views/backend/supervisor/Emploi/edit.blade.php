            ...
            <div class="form-row">
                    ...
                    <select id="classe_id" name="classe_id" class="form-control">
                        @foreach ($classes as $classe)
                            <option value="{{ $classe->id }}"
                                {{ $classRoutine->classe_id == $classe->id ? 'selected' : '' }}>
                ...
                <div class="form-group col-md-12">
                    ...
                    <select id="classroom" name="classroom_id" class="form-control">
                        @foreach ($classrooms as $x)
                            <option value="{{ $x->id }}"
                                {{ $classRoutine->classroom_id == $x->id ? 'selected' : '' }}>
                    ...
                    <select id="subject" name="subject_id" class="form-control">
                        @foreach ($subjects as $subject)
                            <option value="{{ $subject->id }}"
                                {{ $classRoutine->subject_id == $subject->id ? 'selected' : '' }}>
                    ...
                    <select id="teaccher" name="teacher_id" class="form-control">
                        @foreach ($teachers as $teacher)
                            <option value="{{ $teacher->user_id }}"
                                {{ $classRoutine->teacher_id == $teacher->user_id ? 'selected' : '' }}>
                                {{ $teacher->user->prenom }} {{ $teacher->user->prenom }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group col-md-12">
                    ...
                    <input type="time" name="start_time"
                        ...
                        placeholder="début du cours de la salle" value="{{ $classRoutine->start_time }}"
                        required>
                    @error('start_time')
                ...
                <div class="form-group col-md-12">
                    ...
                    <input type="time" name="end_time"
                        ...
                        placeholder="début du cours de la salle" value="{{ $classRoutine->end_time }}"
                        required>
                    @error('end_time')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
